...
module CNFManager
  ...
  def self.export_published_chart(config, cli_args)
    ...
    # Pull new version
    helm_info = Helm.pull(helm_chart_name)
    ...
    tgz_name = get_and_verify_tgz_name(helm_chart_name)

    Log.info { "tgz_name: #{tgz_name}" }

    TarClient.untar(tgz_name, "#{destination_cnf_dir}/exported_chart")

    Log.for("verbose").info { "mv #{destination_cnf_dir}/exported_chart/#{Helm.chart_name(helm_chart_name)}/* #{destination_cnf_dir}/exported_chart" } if verbose
    Log.for("verbose").debug {
      stdout = IO::Memory.new
      Process.run("ls -alR #{destination_cnf_dir}", shell: true, output: stdout, error: stdout)
    ...
    FileUtils.mv(
      Dir.glob("#{destination_cnf_dir}/exported_chart/#{Helm.chart_name(helm_chart_name)}/*"),
  ...
  def self.cnf_to_new_cluster(config, kubeconfig)
    ...
    when CNFInstall::InstallMethod::HelmChart
      ...
      helm_chart_name = config.deployments.get_deployment_param(:helm_chart_name)
      begin
        helm_install = Helm.install("#{release_name} #{helm_chart_name} --kubeconfig #{kubeconfig} #{helm_namespace_option}")
      rescue e : Helm::CannotReuseReleaseNameError
        stdout_warning "Release name #{release_name} has already been setup."
      end
    when CNFInstall::InstallMethod::HelmDirectory
  ...
  def self.find_tgz_files(helm_chart_name)
    Dir.glob(get_helm_tgz_glob(helm_chart_name))
  end

  def self.get_and_verify_tgz_name(helm_chart_name)
    tgz_files = find_tgz_files(helm_chart_name)
    
    if tgz_files.empty?
      Log.error { "No .tgz files found for #{get_helm_tgz_glob(helm_chart_name)}" }
      raise TarFileNotFoundError.new(Helm.chart_name(helm_chart_name))
  ...
  def self.get_helm_tgz_glob(helm_chart_name)
    "#{Helm.chart_name(helm_chart_name)}-*.tgz"
  end
