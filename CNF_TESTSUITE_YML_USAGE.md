...
The following is a basic working example cnf-testsuite.yml file that can be found in the cnf-testsuite respository: [cnf-testsuite/embedded_files/cnf-testsuite-v2-example.yml](https://github.com/cnti-testcatalog/testsuite/blob/main/embedded_files/cnf-testsuite-v2-example.yml)
...
You can quickly generate your own cnf-testsuite.yml dynamically by running `./cnf-testsuite generate_config`.
...
Common parameters required for proper functioning of some tests.
`common: ...`
...
`whitelist_container_names: [coredns]`
...
Helm repository name and url can be omitted, if needed repository is already present locally. 
...
helm_charts:
  - name: coredns  # Name of the deployment
    ...
    helm_chart_name: stable/coredns  # Name of the helm chart in format repo_name/chart_name
...
Deployment, defined by directory with manifest files, that are meant to be directrly installed by Kubernetes.
Explanations with example:
```yaml
manifests:
...
These parameters are needed for RAN, Open5gs and UERANSIM tests
